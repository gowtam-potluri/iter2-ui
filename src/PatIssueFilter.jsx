...
class IssueFilter extends React.Component {
  ...
  onChangeStatus(e) {
    this.setState({ helpreq: e.target.value, changed: true });
  }
  onChangeEffortMin(e) {
  ...
  applyFilter() {
    const { helpreq, effortMin, effortMax } = this.state;
  ...
  render() {
    const { helpreq, changed } = this.state;
    const { effortMin, effortMax } = this.state;
    return (
      <Row>
        <Col xs={6} sm={4} md={3} lg={2}>
          <FormGroup>
            <ControlLabel>Help Required:</ControlLabel>
            <FormControl
